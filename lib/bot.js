...
const bot = {
  ...
  startAnnouncement(userId, channelId, triggerId) {
    ...
    announcementForm = this.getAnnouncementForm(`previewAnnouncement:${channelId}`);

    axios.post(this.web.slackAPIUrl + 'forms.send', {
      token: process.env.SLACK_CLIENT_TOKEN,
      trigger_id: triggerId,
      form: JSON.stringify(announcementForm),
    }).then((res) => {
      //console.log(res);
    }).then(() => {
      return Promise.resolve({ text: 'Blergh!' });
    });
  },
  ...
  getAnnouncementForm(callback_id = null, title = '', details = '') {
    return {
      'title': 'Make an announcement',
      'callback_id': callback_id,
      'submit_label': 'Make Request',
      'elements': [
        {
          'type': 'text',
          'name': 'title',
