...
const bot = {
  ...
  startAnnouncement(userId, triggerId) {
    ...
    axios.post(this.web.slackAPIUrl + 'forms.send', {
      ...
      form: JSON.stringify(this.getAnnouncementForm(`previewAnnouncement:${userId}`)),
    }).then((res) => {
      console.log(res);
    }).then(() => Promise.resolve({ text: 'Blergh!' }));
  },
  ...
  getAnnouncementForm(callback_id = null) {
    return {
